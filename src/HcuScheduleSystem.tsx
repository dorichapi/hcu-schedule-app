      ...
      activeNurses.forEach(nurse => {
        ...
        for (let d = 0; d < daysInMonth - 5; d++) {
          if (adjustedSchedule[nurse.id][d] === '夜' && adjustedSchedule[nurse.id][d + 1] === '明' &&
            ...
            if (d + 5 < daysInMonth && adjustedSchedule[nurse.id][d + 5] !== '休') {
        ...
        if (nightCount > maxN) {
          for (let d = daysInMonth - 1; d >= 0 && nightCount > maxN; d--) {
            if (adjustedSchedule[nurse.id][d] === '夜') {
              ...
              if (d + 2 < daysInMonth && adjustedSchedule[nurse.id][d + 2] === '休') {
                adjustedSchedule[nurse.id][d + 2] = '日';
              }
              nightCount--;
            }
          }
...
export default HcuScheduleSystem;import React, { useState, useEffect, useMemo, useRef } from 'react';
import { Calendar, Settings, Moon, Sun, Clock, RefreshCw, AlertCircle, CheckCircle, Plus, Trash2, LogOut, Lock, Download, Upload, Edit2, Save, X, Eye, Users, FileSpreadsheet, Activity, Maximize2, Minimize2 } from 'lucide-react';
import * as XLSX from 'xlsx';
import { supabase } from './lib/supabase';
