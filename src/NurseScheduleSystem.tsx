...
const DEPARTMENTS = {
  HCU: { name: 'HCU', label: 'HCU', gradient: 'from-indigo-500 to-purple-600' },
  救急外来: { name: '救急外来', label: '救急外来', gradient: 'from-red-500 to-orange-600' }
};
...
const saveSchedulesToDB = async (year: number, month: number, scheduleData: Record<number, (string | null)[]>, deptNurseIds?: number[]) => {
  const deleteQuery = supabase.from('schedules').delete().eq('year', year).eq('month', month);
  if (deptNurseIds && deptNurseIds.length > 0) {
    await deleteQuery.in('nurse_id', deptNurseIds);
  } else {
    await deleteQuery;
  }
  const rows: any[] = [];
...
const NurseScheduleSystem = () => {
  ...
  // 部門選択
  const [selectedDepartment, setSelectedDepartment] = useState<string>('HCU');
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 職員別シフト設定の読み込み（部門別）
        const savedPrefs = await fetchSettingFromDB(`nurseShiftPrefs-${selectedDepartment}`);
        if (savedPrefs) {
          try {
  ...
  const activeNurses = useMemo(() =>
    nurses.filter(n => n.active && n.department === selectedDepartment).sort((a, b) =>
      (POSITIONS[a.position]?.priority || 99) - (POSITIONS[b.position]?.priority || 99)
    ), [nurses, selectedDepartment]);

  // スタッフログイン用: 全部門の職員リスト
  const allActiveNurses = useMemo(() =>
    nurses.filter(n => n.active), [nurses]);
  const allNursesWithCodes = useMemo(() =>
    allActiveNurses.map(n => ({
      ...n,
  ...
  const applyExcelImport = () => {
    ...
    // 他部門の職員を保持しつつ、現部門の職員を入れ替え
    const otherDeptNurses = nurses.filter(n => n.department !== selectedDepartment);
    setNurses([...otherDeptNurses, ...newNurses]);
    // DB一括保存（現部門の職員のみ削除して再挿入）
    (async () => {
      try {
        await supabase.from('nurses').delete().eq('department', selectedDepartment);
    ...
    (async () => {
      try {
        ...
        await supabase.from('schedules').delete()
          ...
          .in('nurse_id', deptNurseIds);
        console.log('前月データ反映のため勤務表を消去しました（部門スコープ）');
  ...
  const exportToExcel = () => {
    ...
    const scheduleData = [
      [`${DEPARTMENTS[selectedDepartment]?.label || selectedDepartment} ${targetYear}年${targetMonth + 1}月 勤務表`],
    ...
    const deptLabel = DEPARTMENTS[selectedDepartment]?.label || selectedDepartment;
    const fileName = `${deptLabel}_勤務表_${targetYear}年${targetMonth + 1}月_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const handleStaffLogin = async () => {
    const nurse = allNursesWithCodes.find(n => n.accessCode === staffCode);
    if (nurse) {
      setStaffNurseId(nurse.id);
      setSelectedDepartment(nurse.department || 'HCU');
      setStaffError('');
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center p-6">
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-10 w-full max-w-lg border border-white/50">
          <div className="text-center mb-10">
            ...
            <p className="text-xs text-gray-400 mt-1">※ 管理者はダッシュボードで部門を切替できます</p>
            <p className="text-xs text-gray-400">※ 職員はアクセスコードで自動的に部門が判別されます</p>
          </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-5 mb-6 border border-white/50">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded-lg text-sm font-bold text-white bg-gradient-to-r ${DEPARTMENTS[selectedDepartment]?.gradient || 'from-gray-400 to-gray-500'}`}>
                  {DEPARTMENTS[selectedDepartment]?.label || selectedDepartment}
                </span>
                <p className="text-lg font-bold text-indigo-600">{targetYear}年{targetMonth + 1}月</p>
              </div>
            </div>
                      ...
                      (async () => {
                        try {
                          ...
                          await supabase.from('schedules').delete()
                            .eq('year', targetYear).eq('month', targetMonth)
                            .in('nurse_id', deptNurseIds);
                          console.log('勤務表データを消去しました（部門スコープ）');
